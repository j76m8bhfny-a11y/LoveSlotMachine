...
Page({
      ...
      success: (res) => {
        ...
        Promise.all(promises).then(results => {
          ...
          if (bestPlace) {
            ...
            // ❌ 没找到，继续试下一批
            console.warn(`[Batch] ${batchKeywords} 无合适结果`);
            this.executeNextStrategy();
          }
        }).catch(err => {
          console.error("Batch Error:", err);
          this.executeNextStrategy(); // 出错也继续，不要卡死
        });
      },
      fail: () => {
  ...
  isValidDateSpot(place, searchKeyword) {
    ...
    const type = (place.type || '').toLowerCase(); // 如 "风景名胜;公园广场;..."
    const address = (place.address || '').toLowerCase();

    // ===========================================
    // 1. ⛔️ 必须死黑名单 (拦截邮政、学校、办事处)
    // ===========================================
    const blackList = [
      '银行', 'atm', '营业厅', '中介', '房产', '物流', '快递', '驿站', '邮政',
      '公司', '厂', '园区', '大厦', '办事处', '委员会', '党群', '居委会', '服务中心',
      '学校', '培训', '驾校', '幼儿园', '小学', '中学', '大学',
      '派出所', '公安', '政府', '法院', '医院', '卫生院', '药房',
      '公厕', '垃圾', '停车场', '收费站', '加油站', '加水', '维修', '汽修',
      '沙县', '拉面', '黄焖鸡', '瑞幸', 'luckin', '蜜雪', '肯德基', '麦当劳',
      '宾馆', '酒店', '招待所' // 除非是度假村，否则普通宾馆不要
    ];
    
    // 命中黑名单直接杀
    if (blackList.some(bad => name.includes(bad) || type.includes(bad))) {
      // console.log(`[淘汰] 黑名单: ${name}`);
      return false; 
    }

    // ===========================================
    // 2. 🟢 绿色通道 (只要类型对，名字没关键词也放行)
    // ===========================================
    // 解决痛点：搜"爬山"出来"明孝陵"(风景名胜)，因为没"山"字被误杀
    ...
    if (safeCategories.some(cat => type.includes(cat))) {
      // 额外校验：如果是"咖啡"，名字不能是"瑞幸"（黑名单已防），但可以防一下其他的
      return true;
    }
    ...
    // 3. 🟡 关键词兜底
    // ===========================================
    // 如果类型比较偏（比如"生活服务"里的"DIY手工"），再检查名字
    const simpleKey = searchKeyword.replace('爬', '').replace('去', '').replace('玩', '');
    if (name.includes(simpleKey)) {
      return true;
    }

    // console.log(`[淘汰] 类型不符且名字无匹配: ${name} [${type}]`);
    return false;
  },
