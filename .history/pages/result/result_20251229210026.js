...
Page({
      ...
      success: (res) => {
        ...
        locationService.searchByType(currentPack.types, userLoc).then(data => {
            if (data && data.poisData && data.poisData.length > 0) {
                // 4. ⚔️ 选妃环节 (评分排序)
                // 这里不再需要 isValidDateSpot，而是用 rankPois 选最好的
                ...
                if (bestPlace) {
                    ...
                    console.warn(`[Result] ${currentPack.name} 结果虽有，但评分/预算不符`);
  ...
  /**
   * 🏆 V6.0 核心排名算法 (替代了 isValidDateSpot)
   * 作用：从一堆“真地点”里，选出“好地点”
   */
  ...
  reRoll() {
    this.setData({ 
      currentIndex: this.data.currentIndex, // 注意：executeNextStrategy 里会自动 +1，这里保持即可
      showReceipt: false,
      isLoading: true
    });
    this.pullLever();
  ...
  generateKeywordQueue(data) { /* 已废弃 */ },
  isValidDateSpot(place, keyword) { /* 已废弃 */ },
