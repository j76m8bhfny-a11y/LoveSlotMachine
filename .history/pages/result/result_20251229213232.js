...
Page({
  data: {
    ...
    historyIds: [], // 存已经推过的店名，比如 ['南京博物院', '红山动物园']
  ...
  startSlotProcess(data) {
    ...
    // ⚠️ 注意：这里不要清空 this.data.historyIds
    // 这样用户点“换一个”时，系统才知道哪些不能推
        ...
        locationService.searchByType(currentPack.types, userLoc).then(data => {
            if (data && data.poisData && data.poisData.length > 0) {
                ...
                if (sortedCandidates.length > 0) {
                    ...
                    // 如果去重后没店了 (比如一共就2个动物园，都推过了)
                    console.warn(`[Result] ${currentPack.name} 里的店都推完了或被过滤了`);
                    this.executeNextStrategy(); // 找下一个策略包 (如从动物园转到游乐场)
                }
            } else {
                this.executeNextStrategy();
            }
  ...
  rankPois(pois, budget, allowedTypes) {
    ...
    pois.forEach(p => {
        // 0. 🧠 记忆去重 (核心修改)
        // 如果这个店已经在记忆库里了，直接跳过！
        if (historyIds.includes(p.name)) {
             console.log(`[去重] 刚才推过，跳过: ${p.name}`);
             return; 
        }
