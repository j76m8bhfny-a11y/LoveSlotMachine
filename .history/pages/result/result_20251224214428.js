...
Page({
  ...
  pullLever(data) {
    ...
    setTimeout(() => {
      ...
      wx.vibrateShort({ type: 'heavy' }); // 触底震动
  ...
  startAnalysisSimulation(data) {
    ...
    // 1. 关系
    if (data.relation && data.relation.includes('初识')) {
      logs.push({ type: 'relation', text: '破冰模式启动！拒绝尴尬～' });
    } else if (data.relation && data.relation.includes('热恋')) {
      logs.push({ type: 'relation', text: '检测到高甜反应！寻找私密角落...' });
    } else {
      logs.push({ type: 'relation', text: `正在为${data.relation || '你们'}定制专属浪漫...` });
    }
